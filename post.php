...
if (isset($_POST['delete'])) {
	...
	if (!$dropped_post) {
		...
		$pass = false;
        if (isset($_COOKIE['pass']) && isset($_COOKIE['uuid'])) {

            $query = prepare("SELECT `pass_key` FROM pass WHERE uuid=:uuid");
            $query->execute(array(':uuid' => $_COOKIE['uuid']));
        ...
        if($board['uri'] == 'vip' && $pass == false){
            error('You need a pass to post here.');
        }

		// Check for CAPTCHA right after opening the board so the "return" link is in there
		if ($config['recaptcha'] && !$pass) {
