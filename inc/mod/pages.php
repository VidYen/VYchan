...
function mod_ads(){
    ...
    $query = prepare("SELECT `id`, `file_location`, `ad_link` FROM advertisement WHERE `ends_at` > :curr_time and `file_location` != '' and `ad_link` != ''");
    $query->execute(array(':curr_time' => time()));
    $rows = $query->fetchAll();

    if(isset($_POST['delete']) && isset($_POST['checkbox'])){
        foreach($_POST['checkbox'] as $id) {
            $query = prepare("DELETE FROM advertisement WHERE `id` = :id");
            $query->execute(array(':id' => $id));
        }
        header('Location: /mod.php?/ads');
    }

    mod_page(_('ads'), 'mod/ads.html', array(
